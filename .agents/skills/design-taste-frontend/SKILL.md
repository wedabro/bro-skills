...
- Always prefer existing framework/theme classes such as `p-4`, `text-lg`,
  `gap-4`, and `rounded-md` over arbitrary values. Use `gap-4` as the primary
  gap; smaller or larger gap classes require a clear hierarchy reason.
  Fixed-pixel exceptions are limited to hairline borders, blur/shadow tuning,
  and very small precision radii; promote repeated exceptions to named tokens.
